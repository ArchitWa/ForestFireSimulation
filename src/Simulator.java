public class Simulator {
    ...
    public void initialize(int treeDensity) {
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                forest[row][col].setTypeOfTree(livingTree);
                treesPlaced++;
                if (treesPlaced == treesToFill) return;
            }
        }
    ...
    /**
     * Shuffles the forest grid
     */
    public void shuffleForest() {
        for (int a = 0; a < rows; a++) {
            for (int b = 0; b < cols; b++) {
                int nA = (int)(Math.random() * rows);
                int nB = (int)(Math.random() * cols);
                int tempLoc = forest[a][b].getTypeOfTree();
                forest[a][b].setTypeOfTree(forest[nA][nB].getTypeOfTree());
                forest[nA][nB].setTypeOfTree(tempLoc);
            }
        }
    }
